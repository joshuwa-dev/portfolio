...
export async function POST(req) {
  try {
    ...
    try {
      // Only emit unlock/login events when a lock actually existed.
      ...
      if (lockSnap && lockSnap.exists) {
        ...
        try {
          ...
          // Do not emit login.success here; it will be emitted unconditionally
          // after the lock-check so we consistently log successful sign-ins.
        } catch (e) {
          console.warn("postAuthLog unlock failed:", e?.message || e);
        }
      } else {
        console.log(
          "unlock endpoint: no lock found for user, skipping auth.account.unlocked emit",
    ...
    // Always emit auth.login.success to indicate a completed Google sign-in.
    try {
      const host = req.headers.get("host") || "localhost:3000";
      const proto =
        req.headers.get("x-forwarded-proto") ||
      ...
      console.log("EMIT auth.login.success (unlock endpoint) ->", JSON.stringify(successPayload));
      await fetch(url, {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      console.warn("postAuthLog login.success failed:", e?.message || e);
    }
