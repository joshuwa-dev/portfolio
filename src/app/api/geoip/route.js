...
const IPAPI_TIMEOUT_MS = process.env.GEOIP_TIMEOUT_MS
  ? Number(process.env.GEOIP_TIMEOUT_MS)
  : 3000;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      cache.set(ip, {
        ...
        fetchedAt: now(),
        expiresAt: shortExpire,
      });
      return { ok: false, status: res.status, text };
    }
    const json = await res.json().catch(() => ({}));
    const country =
      json && (json.country || json.country_name || json.country_code)
        ? json.country || json.country_code || json.country_name
        : null;
    const entry = {
      country,
      raw: json,
...
export async function GET(req) {
  ...
  const clientIp =
    qIp || (forwarded ? forwarded.split(",")[0].trim() : null) || "8.8.8.8";
