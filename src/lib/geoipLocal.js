...
const ASN_DB_PATH =
  process.env.GEOIP_ASN_DB_PATH ||
  path.join(process.cwd(), "data", "GeoLite2-ASN.mmdb");
...
async function tryLoad() {
  ...
  try {
    ...
    console.warn(
      "geoipLocal: unexpected error during DB load",
      err?.message || err,
    );
  }
}
...
export async function lookup(ip) {
  ...
  const countryCode =
    cityRes?.country?.iso_code || cityRes?.registered_country?.iso_code || null;
  const countryName =
    cityRes?.country?.names?.en ||
    cityRes?.registered_country?.names?.en ||
    null;
  const cityName = cityRes?.city?.names?.en || null;
  const subdivision =
    cityRes?.subdivisions && cityRes.subdivisions[0]
  ...
  const isp =
    asnRes?.traits?.isp ||
    asnRes?.isp ||
    asnRes?.autonomous_system_organization ||
    cityRes?.traits?.isp ||
    null;
  const organization =
    asnRes?.autonomous_system_organization ||
    asnRes?.traits?.autonomous_system_organization ||
    asnRes?.traits?.organization ||
    cityRes?.traits?.organization ||
    null;
  ...
  return {
    ...
    organization,
    source:
      (cityLoaded ? "maxmind-city" : "") +
      (asnLoaded ? (cityLoaded ? "+asn" : "maxmind-asn") : ""),
